...
export default function ReportManagement() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-bold text-slate-900">Handle Reports & Disputes</h2>
        <p className="text-sm text-slate-500">Queue for report and dispute resolution</p>
      </div>
      ...
      {tab === 'reports' ? (
                    ...
                    <td className="px-4 py-3 text-right">
                      {r.status === 'PENDING' || r.status === 'IN_REVIEW' ? (
                        <button onClick={() => setSelectedReport(r.id)}
                          className="text-xs text-blue-600 hover:underline">Resolve</button>
                      ) : (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          {disputesLoading ? (
            ...
            <div className="text-center py-12 text-slate-400 text-sm">No disputes</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b">
                <tr>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg shadow-xl">
            <h3 className="font-semibold text-slate-900 mb-4 flex items-center gap-2">
              <AlertTriangle className="w-5 h-5 text-amber-500" /> Resolve Report
            </h3>
            <textarea value={resolution} onChange={(e) => setResolution(e.target.value)}
              placeholder="Notes / decision..." rows={3}
            ...
            <select value={action} onChange={(e) => setAction(e.target.value)}
              ...
              <option value="HIDE_JOB">Hide job</option>
              <option value="REFUND">Refund</option>
              <option value="RELEASE_FUNDS">Release escrow</option>
              <option value="WARNING">Warning</option>
            </select>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setSelectedReport(null)} className="px-4 py-2 text-sm bg-slate-100 rounded-xl">Cancel</button>
              <button onClick={() => handleResolve('DISMISSED')} className="px-4 py-2 text-sm bg-slate-600 text-white rounded-xl">Dismiss</button>
              <button onClick={() => handleResolve('RESOLVED')} disabled={resolveReport.isPending}
                className="px-4 py-2 text-sm bg-blue-600 text-white rounded-xl disabled:opacity-50">Confirm</button>
            </div>
          </div>
        </div>
      )}
