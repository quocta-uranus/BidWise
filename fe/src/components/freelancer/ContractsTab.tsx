...
export default function ContractsTab() {
  ...
  const [clientAnonymous, setClientAnonymous] = useState(false);
  ...
  const handleSubmitDeliverable = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedContract || !submittingMilestone || !selectedFile) return;

    await submitMilestoneDeliverable(selectedContract.id, submittingMilestone.id, selectedFile, fileNote);
  ...
  const handleDownloadDeliverable = async (contractId: string, milestoneId: string, fileName: string) => {
    try {
      const response = await apiClient.get(`/contracts/${contractId}/milestones/${milestoneId}/download`, {
        responseType: 'blob',
      });
      ...
      console.error('Không thể tải file nộp bài:', error);
      alert(language === 'vi' ? 'Không thể tải file nộp bài.' : 'Failed to download deliverable.');
    }
  };
  ...
  const handleClientReviewSubmit = async (e: React.FormEvent) => {
    ...
    const res = await reviewFreelancer(clientReviewingContractId, {
      qualityRating: flSkillRating,
      speedRating: flSpeedRating,
      commRating: flCommRating,
      comment: clientReviewNote,
      anonymous: clientAnonymous,
    });
    ...
    if (res.success) {
      ...
      setTimeout(() => {
        ...
        setFlCommRating(5);
        setClientReviewNote('');
        setClientAnonymous(false);

        // Sync local selected contract state
        if (selectedContract?.id === clientReviewingContractId) {
          setSelectedContract({ ...selectedContract, freelancerReviewed: true });
        }
      }, 1200);
    } else {
      alert(res.error || (language === 'vi' ? 'Không thể gửi đánh giá.' : 'Failed to submit review.'));
    }
  };
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
            ...
            <div className="flex justify-between items-start gap-4 border-b border-slate-100 pb-4">
              ...
              <div className="flex flex-col items-end gap-2 shrink-0">
                ...
                {selectedContract.status === 'COMPLETED' &&
                  !selectedContract.freelancerReviewed &&
                  currentRole === 'CLIENT' && (
                    <button
                  ...
                  {selectedContract.milestones.map((ms, idx) => {
                    ...
                    return (
                      <div key={ms.id} className="border border-slate-100 rounded-2xl p-4 bg-slate-50/40 space-y-3.5 hover:border-slate-200 transition-colors">
                        ...
                        {/* Deliverable display and Approve action */}
                        {ms.deliverable && (
                          <div className="space-y-2 bg-slate-50 border border-slate-200/60 p-3 rounded-xl text-[10px] font-semibold leading-relaxed shadow-sm">
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-1.5 min-w-0">
                                <FileText className="w-4 h-4 text-blue-500 shrink-0" />
                                <span className="font-extrabold text-slate-750 truncate max-w-[120px]" title={ms.deliverable}>
                                  {ms.deliverable}
                                </span>
                              </div>
                              <button
                                onClick={() => handleDownloadDeliverable(selectedContract.id, ms.id, ms.deliverable!)}
                                className="px-2 py-1 bg-blue-50 hover:bg-blue-100 text-blue-650 rounded-md transition-colors flex items-center gap-1 shrink-0 font-bold"
                              >
                            ...
                            {ms.deliverableDesc && (
                              <p className="text-slate-450 border-t border-slate-200/50 pt-1.5 mt-1.5">
                                {t('contracts.noteLabel')} <span className="text-slate-655 italic">"{ms.deliverableDesc}"</span>
                              </p>
                            )}

                            {/* Client Approval Action */}
                            {ms.status === 'SUBMITTED' && isClient && (
                              <button
            ...
            <form onSubmit={handleSubmitDeliverable} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-650">
                  {language === 'vi' ? 'Chọn file nộp bài' : 'Select delivery file'}
                </label>
                <div className="border-2 border-dashed border-slate-200 hover:border-blue-500 rounded-xl p-4 transition-all flex flex-col items-center justify-center cursor-pointer relative bg-slate-50/50 hover:bg-slate-50">
                  <input
                    type="file"
                    required
                    onChange={(e) => {
                  ...
                  <div className="flex flex-col items-center text-center space-y-1.5 pointer-events-none">
                    ...
                    {selectedFile ? (
                      <div className="text-xs font-bold text-slate-700">
                        {selectedFile.name} <span className="text-[10px] text-slate-400 font-normal">({(selectedFile.size / 1024 / 1024).toFixed(2)} MB)</span>
                      </div>
                    ) : (
                      <>
                        <span className="text-xs text-slate-550 font-bold">
                          {language === 'vi' ? 'Kéo thả hoặc Nhấp để chọn file' : 'Drag & drop or click to choose file'}
                        </span>
                        <span className="text-[10px] text-slate-400">
                          {language === 'vi' ? 'Hỗ trợ Zip, PDF, PNG, JPG (Tối đa 50MB)' : 'Supports Zip, PDF, PNG, JPG (Max 50MB)'}
                        </span>
                      </>
                    )}
                  </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className={`h-10 px-4 rounded-xl font-bold text-sm shadow-sm transition-all ${
                    selectedFile
              ...
              <form onSubmit={handleClientReviewSubmit} className="space-y-4 text-xs font-semibold">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="rounded text-blue-600 focus:ring-blue-500/20 w-4 h-4 transition-all"
                  />
                  <label htmlFor="client-anon" className="text-slate-500 font-bold cursor-pointer">
                    {language === 'vi' ? 'Đánh giá ẩn danh' : 'Review anonymously'}
                  </label>
                </div>
