...
import { useFreelancer, Bid, Job } from '@/lib/hooks/useFreelancer';
import { useTranslation } from '@/lib/i18n/useTranslation';
import { getJobTitle } from '@/lib/i18n/demo-content';
import CreateJobModal from './CreateJobModal';

export default function ClientJobsTab() {
  const { jobs, bids, simulateClientAcceptBid, deleteJob } = useFreelancer();
  ...
  const handleDeleteJob = (jobId: string) => {
    if (!window.confirm(t('jobs.deleteJobConfirm'))) return;
    deleteJob(jobId);
  ...
  const categoryLabel: Record<string, string> = {
    ...
    fullstack: language === 'vi' ? 'Fullstack' : 'Fullstack',
    mobile:    language === 'vi' ? 'Mobile'    : 'Mobile',
  };

  const categoryColor: Record<string, string> = {
    frontend:  'bg-blue-50 text-blue-700 border-blue-100',
    backend:   'bg-violet-50 text-violet-700 border-violet-100',
    fullstack: 'bg-indigo-50 text-indigo-700 border-indigo-100',
                  ...
                  >
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${
                          isAccepted ? 'bg-green-50 text-green-700 border-green-150' : 'bg-blue-50 text-blue-700 border-blue-150'
                        }`}>
                          {isAccepted ? (language === 'vi' ? 'Đã giao' : 'Assigned') : (language === 'vi' ? 'Mở thầu' : 'Open')}
                        </span>
                      </div>
                    </div>

                    <div className="flex items-center gap-1.5 mt-1.5">
                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${categoryColor[job.category]}`}>
                    ...
                    <div className="flex justify-between items-center mt-2.5 pt-2 border-t border-slate-100 text-[10px]">
                      <span className="text-slate-500">
                        {t('jobs.budget')}: <strong className="text-slate-800">${job.budget.toLocaleString()}</strong>
                      </span>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-blue-600">
                          {jobBids.length} {language === 'vi' ? 'đề xuất' : 'bids'}
                        </span>
                        <button
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2 text-[10px] text-slate-500 font-semibold">
                      <span className={`px-2 py-0.5 rounded border font-bold ${categoryColor[selectedJob.category]}`}>
                        {categoryLabel[selectedJob.category]}
                      </span>
                      <span>{selectedJob.auctionType === 'SEALED' ? '🔒 Sealed Bid' : '🔓 Open Bid'}</span>
                      <span>📅 {language === 'vi' ? 'Hạn:' : 'Deadline:'} {selectedJob.deadline}</span>
                      <span>💰 ${selectedJob.budget.toLocaleString()} USD</span>
                    </div>
                  </div>
