...
export default function DashboardPage() {
  ...
  const freelancerStats = [
    { label: t('dashboard.statOpenJobs'), value: String(jobs.length), icon: <Briefcase className="w-7 h-7 text-blue-500" /> },
    { label: t('dashboard.statBidsSent'), value: String(activeBids.length), icon: <Target className="w-7 h-7 text-violet-500" /> },
    { label: t('dashboard.statActiveContracts'), value: String(activeContracts.length), icon: <FileCheck className="w-7 h-7 text-emerald-500" /> },
    { label: t('dashboard.statTotalEarnings'), value: `$${wallet.totalEarned}`, icon: <DollarSign className="w-7 h-7 text-amber-500" /> },
  ];
  ...
  const clientStats = [
    { label: language === 'vi' ? 'Dự án đã đăng' : 'Jobs Posted', value: String(clientJobs.length), icon: <Briefcase className="w-7 h-7 text-blue-500" /> },
    { label: language === 'vi' ? 'Số thầu nhận được' : 'Bids Received', value: String(totalBidsReceived), icon: <Target className="w-7 h-7 text-violet-500" /> },
    { label: language === 'vi' ? 'Hợp đồng đang chạy' : 'Active Contracts', value: String(activeContracts.length), icon: <FileCheck className="w-7 h-7 text-emerald-500" /> },
    { label: language === 'vi' ? 'Đang giữ Ký quỹ (Escrow)' : 'Escrow Hold', value: `$${wallet.escrow}`, icon: <Shield className="w-7 h-7 text-orange-500" /> },
  ];
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-t border-indigo-950/40 bg-slate-950/20 text-[9px] text-slate-500 font-semibold space-y-2">
        {activeRole === 'FREELANCER' ? (
          ...
          <div className="flex justify-between items-center bg-slate-900/40 px-2 py-1.5 rounded-lg border border-indigo-950/20">
            <span className="flex items-center gap-1.5"><Shield className="w-3 h-3 text-emerald-500" /> Escrow Guarantee</span>
        ...
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          <div className="relative flex-1 flex flex-col max-w-xs w-full bg-gradient-to-b from-slate-900 to-indigo-950 text-slate-200 border-r border-indigo-900/30 animate-in slide-in-from-left duration-300">
            <div className="absolute top-2 right-2">
              <button
                onClick={() => setIsMobileMenuOpen(false)}
                className="w-8 h-8 rounded-full bg-slate-850 hover:bg-slate-800 text-slate-400 hover:text-white flex items-center justify-center"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {stats.map((stat) => (
                    <div key={stat.label} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs hover:border-slate-300 hover:shadow-sm transition-all duration-200">
                      <div className="mb-3 p-2 w-fit rounded-xl bg-slate-50">{stat.icon}</div>
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-2.5">
                      ...
                      <div className="space-y-2 text-[10px] text-slate-500 leading-relaxed font-semibold">
                        <div className="flex items-start gap-2">
                          <Megaphone className="w-3.5 h-3.5 text-blue-500 mt-0.5 shrink-0" />
